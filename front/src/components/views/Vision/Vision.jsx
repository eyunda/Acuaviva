...
const Vision = () => {
	...
	const init = async () => {
		const { data } = await ApiRequest().get('/vision')
		setVisionList(data)
	}

	const columns = [
		{ field: 'id', headerName: 'ID', width: 50 },
		{ field: 'vision', headerName: 'Vision', width: 800 },
		{
			field: '',
			headerName: 'Acciones',
			width: 150,
	...
	const onDelete = async (id) => {
		try {
			const { data } = await ApiRequest().post('/vision/eliminar', { id: id })
			setMensaje({
	...
	const onSubmit = async () => {
		try {
			const { data } = await ApiRequest().post('/vision/guardar', body)
			handleDialog()
	...
	const onEdit = async () => {
		try {
			const { data } = await ApiRequest().post('/vision/editar', body)
			handleDialog()
			setBody(initialState)
			setMensaje({
				ident: new Date().getTime(),
	...
	return (
		<>
			...
			<Dialog maxWidth='xs' open={openDialog} onClose={handleDialog}>
				<DialogTitle>
					{isEdit ? 'Editar Vision' : 'Crear Vision'}
				</DialogTitle>
				<DialogContent>
				...
				<Container maxWidth='lg'>
					...
					<Grid container spacing={2}>
						...
						<Grid item xs={12} sm={12}>
							<CommonTable data={visionList} columns={columns} />
						</Grid>
					</Grid>
				</Container>
			</Page>
