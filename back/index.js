...
app.post('/api/cliente', (req, res) => {
	...
	const values = [username, password, rol]
	var connection = mysql.createConnection(credentials)
	connection.query("SELECT * FROM clientes c, rol r WHERE c.username = ? AND c.password = ? AND c.id_rol = r.id AND c.id_rol='2'", values, (err, result) => {
		if (err) {
			res.status(500).send(err)
		} else {
			if (result.length > 0) {
...
app.get('/api/factura', (req, res) => {
	var connection = mysql.createConnection(credentials)
	connection.query("SELECT nombre, apellido, edad, telefono, estrato, descuento,costos, valor, lectura_resiente-lectura_pasada Consumo,  deuda_anterior, lectura_resiente, lectura_pasada, ((((lectura_resiente-lectura_pasada)*valor)-descuento)+deuda_anterior) Total FROM clientes c, recibo r, estrato e, costos_fijos co WHERE c.username = 'sofi' AND c.id_estrato=e.id AND r.id_cliente = c.id", (err, rows) => {
		if (err) {
			res.status(500).send(err)
		} else {
...
app.post('/api/guardar', (req, res) => {
	...
	const params = [[nombre, apellido, edad, telefono, correo, username, password, id_rol]]
	var connection = mysql.createConnection(credentials)
...
app.post('/api/listacli/guardar', (req, res) => {
	...
	const params = [[nombre, apellido, edad, telefono, correo, username, password, rol]]
	var connection = mysql.createConnection(credentials)
...
app.post('/api/pqrs/guardar', (req, res) => {
	...
	const params = [[pqrs, id_cliente, id_estado]]
	var connection = mysql.createConnection(credentials)
